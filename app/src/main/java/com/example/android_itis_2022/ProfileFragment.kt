        ...
        binding?.run {
            button1.setOnClickListener {
                parentFragmentManager.beginTransaction()
                    ...
                    .replace(
                        R.id.container,
                        ProfileSecondFragment.getInstance(counter.text.toString().toInt()),
                        ProfileSecondFragment.ProfileFragment2_TAG
                    )
                    .addToBackStack(null)
                    .commit()
            }
            ...
            buttonOpenDialogFragment.setOnClickListener {
                ...
                dialog.plus = { dialValue: Int ->
                    count += dialValue
                    binding?.counter?.text = "$count"
                }
                dialog.minus = { dialValue: Int ->
                    count -= dialValue
                    if (count >= 0) {
                        binding?.counter?.text = "$count"
                    } else {
